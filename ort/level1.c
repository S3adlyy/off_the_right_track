...
void game_level1(SDL_Surface *ecran, int *page){
	...
	int fd = open("/dev/ttyUSB0", O_RDWR | O_NOCTTY | O_NDELAY);
    	...
    	cfsetispeed(&options, B115200); //add to lvl 1
	...
	Text txt_playerScore= init_text(55, 0.24*SCREEN_H, "fonts/emporia-roman.otf", 50, 255, 255, 255);
	SDL_Event event;
	//init music
	...
	loadSettings(&volumeMusic, &volumeSfx);
        init_music(level1theme, "game/egyptlvl1.wav");
	...
	dash_sfx= init_chunk("game/dash_sfx.wav");
        ...
        //init chapter intro
        init_image(&intro_img_spritesheet, "game/intro_lvl1_spritesheet.png", SCREEN_W-984, SCREEN_H*0.27);
	...
	for(i=0;i<4;i++){
        ...
        //init UI
        Image scroll_btn, tutorial_btn, gamewon_img, score_ui;
        init_image(&gamewon_img, "game/gamewon_lvl1.png", 0,0);
	...
	init_image(&score_ui, "game/score_ui.png", -30, 0.2*SCREEN_H);
	//init health bar
	...
	//init objectives
	init_objective(&obj[0], "Collect Points of Influence" , 30, 0.3*SCREEN_H);
	init_objective(&obj[1], "Eliminate 5 Mummies" , 5, 0.35*SCREEN_H);
	init_objective(&obj[2], "Collect 4 Documents" , 4, 0.4*SCREEN_H);
	
	//init memory game
	Image memorycardimg1, memorycardimg2, memorycardimg3;
	init_image(&memorycardimg1, "game/memorycard1.png", 0, 0);
	init_image(&memorycardimg2, "game/memorycard2.png", 0, 0);
	init_image(&memorycardimg3, "game/memorycard3.png", 0, 0);
	...
	Image pillarsimg;
	init_image(&pillarsimg, "game/pillars.png", 0, 0);
	init_image(&bg, "level1bg.png",0,0);
	init_image(&bgmask, "level1bgmask.png", 0, 0);
	init_minimap(&m, 620, 110, (SDL_Rect){100, 800}, "bglvl1minimap.png");
	...
	if(level1loaded){
		...
		init_player(&p1, 100, 500, 8 ,14);//8->num max frames| 14>num movements/rows
		printf("HP: %d\n", p1.hp);
	}
	...
	if(level1loaded)
		...
		initCamera(&camera, 0, 500, 1920, 1080, 1);
	...
	t_prev = SDL_GetTicks();
      ...
      //NPC JOBS: 1-minigame ortcrush 2- tictactoe 3- Q&A 4- minigame memory card 5- INFORMATIONAL NPC(Only gives information and documents)
        Npc npcs[n_npc];
        npcs[0] = init_npc("game/npc_spritesheet.png", 1 , 540, 410, 1);
         npcs[1] = init_npc("game/npc_spritesheet.png", 3 , 313, 640, 2);
         npcs[2] = init_npc("game/npc_spritesheet.png", 6 , 475, 109, 3);
         npcs[3] = init_npc("game/npc_spritesheet.png", 5 , 2496, 625, 4);
         npcs[4] = init_npc("game/npc_spritesheet.png", 3 , 3746, 578, 5);
         npcs[5] = init_npc("game/npc_spritesheet.png", 2 , 4369, 627, 6);
         npcs[6] = init_npc("game/npc_spritesheet.png", 1 , 4841, 625, 7);
         npcs[7] = init_npc("game/npc_spritesheet.png", 5 , 5684, 639, 8);
         npcs[8] = init_npc("game/npc_spritesheet.png", 3 , 7186, 640, 9);
         npcs[9] = init_npc("game/npc_spritesheet.png", 2 , 4782, 168, 10);
        ...
        gameObjects = (GameObject *)malloc(n_gameObj*sizeof(GameObject));
          init_gameObject(&gameObjects[0], 330, 100, "enigma_minimap_indicator.png",2);
        init_gameObject(&gameObjects[1], 397, 680, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[2], 1042, 538, "game/hp_indicator.png",5);
        init_gameObject(&gameObjects[3], 1422, 430, "game/scroll_indicator_spritesheet.png",7);
        init_gameObject(&gameObjects[4], 1630, 430, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[5], 1714, 280, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[6], 1842, 182, "game/time_indicator.png",6);
        init_gameObject(&gameObjects[7], 2055, 287, "game/scroll_indicator_spritesheet.png",7);
        init_gameObject(&gameObjects[8], 2187, 482, "game/time_indicator.png",6);
        init_gameObject(&gameObjects[9], 2499, 455, "enigma_minimap_indicator.png",9);//mem game
        init_gameObject(&gameObjects[10], 2771, 754, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[11], 2868, 754, "enigma_minimap_indicator.png",1);
        init_gameObject(&gameObjects[12], 3138, 711, "game/time_indicator.png",6);
        init_gameObject(&gameObjects[13], 3268, 738, "game/hp_indicator.png",5);
        init_gameObject(&gameObjects[14], 3109, 712, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[15], 3546, 738, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[16], 3698, 698, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[17], 520, 220, "game/influence_indicator.png",4);
         init_gameObject(&gameObjects[18], 580, 220, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[19], 470, 220, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[20], 849, 420, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[21], 603, 179, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[22], 2345, 720, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[23], 4983, 130, "game/scroll_indicator_spritesheet.png",7);
        init_gameObject(&gameObjects[24], 4225, 770, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[25], 4150, 770, "game/influence_indicator.png",4);
        init_gameObject(&gameObjects[26], 4075, 740, "game/influence_indicator.png",4);
         init_gameObject(&gameObjects[27], 4460, 160, "game/time_indicator.png",6);
          init_gameObject(&gameObjects[28], 4435, 160, "game/influence_indicator.png",4);
           init_gameObject(&gameObjects[29], 4130, 120, "enigma_minimap_indicator.png",1);
	...
	tutorials.active=1;
	tutorials.currentIndexDisplaying = 0;
	//FIN TUTORIAL INIT
	
	//----INIT DOCUMENT SYSTEM
	Documents documents;
	init_documents(&documents, 6);
	documents.T = NULL;
	...
	init_doc(&documents.T[0], "documents/document_1.png", 1,"documents/document_1voice.wav");
	init_doc(&documents.T[1], "documents/document_2.png", 2,"documents/document_2voice.wav");
	init_doc(&documents.T[2], "documents/document_3.png", 3,"documents/document_3voice.wav");
	init_doc(&documents.T[3], "documents/document_4.png", 4,"documents/document_4voice.wav");
	init_doc(&documents.T[4], "documents/document_5.png", 5,"documents/document_5voice.wav");
	init_doc(&documents.T[5], "documents/document_6.png", 6,"documents/document_6voice.wav");
			...
			switch(event.type){
				...
				case SDL_MOUSEBUTTONDOWN:
					...
					if(memorygameactive==1){
						for(i=0;i<6;i++){
							printf("checking click i: %d; flipped:%d\n", i, memorygame_flipped[i]);
	...
	if (timer - oldTimer >= 1000) { // Check if a second has passed
		...
		if (game_time >= 0) {
		    ...
		    char time_string[32];
		    sprintf(time_string, "%d", game_time);
		    
		    printf("%s",time_string);
		    write(fd, time_string, sizeof(time_string));
	...
	if(!pausemenuActive && intro_frames>=4){
		...
		if(p1.status != PLAYER_DEAD && !tutorials.active)											
			move_player(&p1, dt);
	...
	if(!p1.rc && !p1.lc || p1.bc){
		...
		updateCamera(&camera, &p1,  1920, 1080, bg.img->w, bg.img->h);
	}
	...
	afficher_sprite_at_pos(pillarsimg, ecran, &pillarsimg.pos2, &(SDL_Rect){pillarsimg.pos1.x - camera.pos.x, pillarsimg.pos1.y - camera.pos.y});
	...
	for(int k=0;k<n_npc;k++){
		...
		if(npcs[k].isActive && npcs[k].dialogueActive==0)//npc dialogue is finished and waiting for next step (if he has a job -> execute it : do nth)
		{
			...
			if(npcs[k].job_completed && npcs[k].job != 0 && npcs[k].job!=5 && npcs[k].job !=6 && s!=0 && npcs[k].job!=7 ){
				int r;
	...
	switch(update_gameObject(gameObjects, &n_gameObj, p1, camera)){//player interacted with the game obejct
		...
		case 3: p1.isRunning=0;
			...
			p1.pos.x -= p1.lastdx;
			break;
		case 4: update_objective(&obj[0], 1);//test  objective update
			Mix_PlayChannel(3,collect_sfx, 0);
	...
	if(intro_frames<4){
		...
		//afficher_text(ecran, &txt_tictactoe, "START MEMORY");
		printf("hp: %d\n",p1.hp);
	...
	//flip all the cards for 3 seconds before they are flipped again		
	if(memorygameactive && memorygametimestart >0){
		memorygametimestart-=dt;
		printf("memorygametimestart= %d\n", memorygametimestart);
		int cardframetmp[6] = {5, 5,5 , 5,5 ,5 };
		SDL_FillRect(ecran, &(SDL_Rect){0, 0, 1920, 1080}, SDL_MapRGB(ecran->format, 155, 155, 155));
		afficher_memorygame(memorygamerect, memorygame, ecran, cardframetmp);
		for(int c=0; c<6;c++){
    			printf("%d|", memorygame_flipped[c]);
    			if(memorygame_flipped[c]==0){
    				switch(memorygame_answers[c]){
    					case 1: 
    						afficher_sprite_at_pos(memorycardimg1, ecran, NULL, &(SDL_Rect){memorygame[c].pos1.x + 25, memorygame[c].pos1.y + 75});
    						break;
    					case 2: 
    						afficher_sprite_at_pos(memorycardimg2, ecran, NULL, &(SDL_Rect){memorygame[c].pos1.x + 25, memorygame[c].pos1.y + 75});
    						break;
    					case 3: 
    						afficher_sprite_at_pos(memorycardimg3, ecran, NULL, &(SDL_Rect){memorygame[c].pos1.x + 25, memorygame[c].pos1.y + 75});
    						break;
    	...
    	if(memorygameactive==1 && memorygamescore!=6 && memorygametimestart<=0){
    		...
    		printf("\nselectedcard1: %d | selecteccard2: %d\n", selectedcard1, selectedcard2);
    		
    		if(selectedcard1 != -1){
    				switch(memorygame_answers[selectedcard1]){
    					case 1: 
    						afficher_sprite_at_pos(memorycardimg1, ecran, NULL, &(SDL_Rect){memorygame[selectedcard1].pos1.x + 25, memorygame[selectedcard1].pos1.y + 75});
    						break;
    					case 2: 
    						afficher_sprite_at_pos(memorycardimg2, ecran, NULL, &(SDL_Rect){memorygame[selectedcard1].pos1.x + 25, memorygame[selectedcard1].pos1.y + 75});
    						break;
    					case 3: 
    						afficher_sprite_at_pos(memorycardimg3, ecran, NULL, &(SDL_Rect){memorygame[selectedcard1].pos1.x + 25, memorygame[selectedcard1].pos1.y + 75});
    						break;
    		...
    		if(selectedcard2 != -1){
    				switch(memorygame_answers[selectedcard2]){
    					case 1: 
    						afficher_sprite_at_pos(memorycardimg1, ecran, NULL, &(SDL_Rect){memorygame[selectedcard2].pos1.x + 25, memorygame[selectedcard2].pos1.y + 75});
    						break;
    					case 2: 
    						afficher_sprite_at_pos(memorycardimg2, ecran, NULL, &(SDL_Rect){memorygame[selectedcard2].pos1.x + 25, memorygame[selectedcard2].pos1.y + 75});
    						break;
    					case 3: 
    						afficher_sprite_at_pos(memorycardimg3, ecran, NULL, &(SDL_Rect){memorygame[selectedcard2].pos1.x + 25, memorygame[selectedcard2].pos1.y + 75});
    						break;
	...
	if(((p1.status == PLAYER_DEAD && p1.frame >=4) || game_time<0) && !gamewon){
		...
		if(d==0){
			*page = 1;
			...
			printf("restart \n");
			remove("level1.dat");//remove save file and restart from all init positions (test this more)
			*page = 2;
			level_active=0;
		}
	}
	...
	liberer_image(memorycardimg1);
	liberer_image(memorycardimg2);
	liberer_image(memorycardimg3);
